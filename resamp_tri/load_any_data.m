function [datastruct]=load_any_data(filename,zone,xzone,yzone,azo)
...
if(sum(type==[1:3])) %cpx file
  ...
  [rmg,count] = fread(fid,[nx1*2,(yzone(1)-1)],'real*4');
  [rmg,count] = fread(fid,[nx1*2,ny],'real*4');
  status      = fclose(fid);
  rmg = rmg((xzone(1)*2-1):(xzone(2)*2),:);
