...
while(done~=1)
    ...
    for i=1:length(tcx)
        ...
        resampstruct(i).data_std = nanstd(data(goodids));% added for Bayesian inversion

        if resampstruct(i).data_std == 0 % exclude the data_std = 0 cases to avoid any error in Bayesian Inversion
            resampstruct(i).data_std = NaN;
            resampstruct(i).X     = NaN;
            resampstruct(i).Y     = NaN;
            resampstruct(i).data  = NaN;
        end
