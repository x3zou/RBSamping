function [datastruct]=load_any_data(filename,zone,ny1,ny2)
...
datastruct=struct([]);
if(nargin<4)
    disp('need filename zone ny1 ny2');
    return
end
if(ny1>=ny2)
    disp('ny1 must be smaller than ny2')
    return
end
...
if(sum(type==[1:3])) %cpx file
  ...
  status=fseek(fid,nx*2*(ny1-1)*4,-1);
